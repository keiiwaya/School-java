...
public class Billiard implements BilIF {  // ビリヤードのクラス
  ...
  public static void main(String[] args) {
    ...
    try{
      ...
      System.out.print("ビリヤード台の種類は（1:通常, 2:変形）");
      inStr = in.readLine();              // 文字列入力
      int t = Integer.parseInt(inStr);    // 文字列を整数に変換
      Billiard bil = new Billiard(m, n, t);  // ビリヤードのインスタンス生成
  ...
  // コンストラクタ  
  // 第１引数：ビリヤード台の幅、第２引数：ビリヤード台の高さ
  // 第３引数：ビリヤード台の種類（1のとき通常の台、2のとき変形台）
  public Billiard(int m, int n, int t) {
    if (t == 1) {                       // 通常のビリヤード台の場合
      table = new Table (m, n);         // m×nサイズの通常のビリヤード台の
                                        // インスタンスを生成
    } else {                            // 変形ビリヤード台のー場合
      table = new StrangeTable (m, n);  // m×nサイズの変形ビリヤード台の
                                        // インスタンスを生成
    }
    ball = new Ball(table, 0, 0);  // ボールのインスタンスを作成して
                                   // テーブルの左上端に置く
  }

  // ゲームの処理
  public void start(Graphics g) {
    ball.go(1, 1, g);    // ボールを初速度(1, 1)で転がす
    System.out.println("ボールがポケット(" + ball.getX() + ", "
                               + ball.getY() + ")に落ちました。");
