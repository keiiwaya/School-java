...
public class GEFrame extends GPFrame {
  ...
  public GEFrame(String title, String fileName, GraphicEditor ged) {
    ...
    if (ge.init(fileName, gra)) {    // initialize the paint tool
      System.out.println("method init() of the tool executed.");
      repaint();        // repaint
    } else {
      System.out.println("Exit because initialization id failed.");
  ...
  public static void main(String[] args) {
    String fileName = "temp.shape";  // the default file name is "temp.shape"
    if (args.length <= 1 ) {
                  // if the number of arguments is equal to or less than 1
      if (args.length == 1) {  // if the number of arguments is equal to 1
        fileName = args[0];    // let the file name be the first argument
      }
      System.out.println("creating an instace of class GraphicEditor.");
      GraphicEditor ged = new GraphicEditor(); // create an instance of GraphicEditor
      System.out.println("an instace of class GraphicEditor created.");
      System.out.println("file name is " + fileName + ".");
      new GEFrame("Graphic Editor", fileName, ged);
                                       // create an instance of GEFrame
    } else {   // if the number of arguments is more than 1
